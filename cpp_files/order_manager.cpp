...
int AriacOrderManager::PickFromConveyerBefore(std::vector<std::pair<float,float>> drop_poses,std::string product_type,std::string empty_bin)
{	std::string direction;
	...
	for(int i=0;i<3;i++)
	   ...
	   while(1){   //for picking up from pulley
		   	...
		   	//ROS_INFO_STREAM("Time elapsed after start of loop"<<ros::Time::now().toSec()-temp);
		   	// if(ros::Time::now().toSec()-temp>50){
		   	// 	ROS_INFO_STREAM("Part was not picked up in even 10 sec so breaking loop");
		   	// 	return -2;
		   	// }
			if (ros::Time::now().toSec()-temp>75){
	   			ROS_INFO_STREAM("100 secs since parts are being picked up");
	   			goto STOP;
	   			}
...
std::vector <std::pair<std::string,std::vector<geometry_msgs::Pose> > > AriacOrderManager::UpdateLists(osrf_gear::Order order){
	int agv_id=0;
...
void AriacOrderManager::ThrowComponents(std::pair<std::string,geometry_msgs::Pose>diff_list,int agv_id){
	...
	while(1){
		...
		if (diff_list.first=="pulley_part")
			reposition_part_pose_back_.position.z+=0.08;
		...
		if (diff_list.first=="pulley_part")
			reposition_part_pose_back_dash_.position.z+=0.08;
	}
...
void AriacOrderManager::ExecuteOrder() {
    ...
    //Check empty bins
    std::string product_type="disk_part";
    //double temp_time=ros::Time::now().toSec();
    std::string empty_bin="/bin1_frame";
