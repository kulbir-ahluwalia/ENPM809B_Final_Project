    ...
    if (gripper_state_){//--while the part is still attached to the gripper
        ...
        if(arm_id_=="arm1"){
            ...
            {    ROS_INFO_STREAM("Moving towards AGV1...");
                 ...
                 this->Execute();
                 ros::spinOnce();
                 ROS_INFO_STREAM("diff  "<<abs(joint_states_[0]- 1.18));
                 //if (gripper_state_==false) return -1;
                 if (abs(joint_states_[0]-1.18)<0.1)
                    break;
            }
         end_position_[1]+=1.55;
        }
            ...
            {    ROS_INFO_STREAM("Moving towards AGV2...");
                 ...
                 this->Execute();
                 ros::spinOnce();
                 ROS_INFO_STREAM("diff  "<<abs(joint_states_[1]-3.09));
                 //if (gripper_state_==false) return -1;
                 if (abs(joint_states_[1]-3.09)<0.1)
                    break;
            }
            home_joint_pose_[1]-=1.55;
         }
        ...
        //ros::Duration(1).sleep();
        part_pose.position.z+=0.3;         //so that arm remains above before keeping so that we can check where part has fallen
        ...
        if (gripper_state_==false) {
            ...
            if(kit_tray_list_.size()!=list_before.size()){
                ...
                while(1){
                    ...
                    picking_part_pose.position.z-=0.005;
